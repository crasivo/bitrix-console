...
class DbTableShowIndexCommand extends Command
{
    /** @var string */
    public const COMMAND_NAME = 'db:table:show:index';
    ...
    protected function configure()
    {
        ...
        $this->setDescription('Show table indexes');
        ...
        $this->setAliases(['db:table:show:indexes']);
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        try {
            ...
            $sqlString = sprintf(sprintf('SHOW INDEXES FROM `%s` IN `%s`;', $sqlHelper->forSql($input->getArgument('table')), $sqlHelper->forSql($database)));
            $rowsCollection = $connection->query($sqlString)->fetchAll();
            if ($rowsCollection === []) {
                $output->writeln('<error>No indexes found</error>');
