...
{
    /** @var string */
    public const COMMAND_NAME = 'user:deactivate';
    ...
    protected function configure()
    {
        ...
        $this->setDescription('Deactivate user');
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        try {
            ...
            $cUser = new \CUser();
            $res = $cUser->Update((int)$user['ID'], ['ACTIVE' => 'N']);
            if (!$res) {
                throw new \Exception(sprintf('Failed deactivate "%s" user: %s', $id, $cUser->LAST_ERROR));
            }
